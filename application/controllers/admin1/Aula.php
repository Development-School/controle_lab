...
class Aula extends CI_Controller {  /**
  ...
  public function index($ano= '',$mes = ''){
    ...
    $this->load->view('admin1/aula_view',$dados);
    /*
  ...
  public function calendario($ano,$mes){    
    ...
    $prefs['next_prev_url'] = base_url('admin1/aula/index');
    $prefs['day_type'] = 'long';
    $prefs['template'] = '
    ...
    foreach ($dias as $d) {
      if ($d['dia'] < 10) {
        $data += array($d['dia'] => base_url('admin1/aula/reservas_dia').'/'.$ano.$mes.'0'.$d['dia'],);
      } 
      else $data += array($d['dia'] => base_url('admin1/aula/reservas_dia').'/'.$ano.$mes.$d['dia'],);
    }
...
/* Location: ./application/controllers/admin1/Aula.php */